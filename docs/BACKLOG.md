...
- [ ] `review` **비밀번호 재설정 (로그인 후속 — 이번 PR 범위 밖)**
  ...
  같은 링크를 두 번 쓸 수 있다.
  > 2026-08-28 dev-loop: 재확인, 변동 없음. `package.json` dependencies 여전히
  > `next`/`pg`/`react`/`react-dom` 뿐 — 메일/SMS 발송 채널과 자격증명 결정이 여전히
  > 없다(새 라이브러리 설치 금지라 SES/Resend SDK 도입도 불가, 카카오 알림톡도 계약·키가
  ...
  > 2026-08-29 dev-loop: 재확인, 변동 없음(`package.json` dependencies 동일, 채널 결정 없음).
  > 열린 `auto/` PR 2건(#35 전역 내비, #34 장바구니 병합) 있으나 둘 다 이 항목과 무관해 스킵
  > 대상 아님. 아래 나머지 "## 할 일" 항목도 전부 사람 결정(배송비·PG·입금계좌·이미지 저장소)
  > 또는 금지 경로(`ops/`·`.github/workflows/`)에 막혀 있어 대안 항목 없음. 실행 가능한 항목
  > 없어 PR 없이 종료.
...
- [x] `review` **관리자 상품 등록 화면 — A1 이후 상품을 넣을 창구가 없다**
  열린 `auto/` PR 2건(#35 전역 내비, #34 장바구니 병합)이 있었지만 이 항목과 무관해 스킵
  대상 아님. "## 할 일" 상단 항목들은 전부 사람 결정(메일 발송 채널·배송비·PG·입금 절차)
  또는 이미 열린 PR 이 다루고 있어(장바구니 병합·전역 내비) 이 항목이 이번 실행에서 실제로
  착수 가능한 첫 항목이었다.
  원문이 말한 "당분간은 `scripts/gen_seed_sql.mjs` 로 버틴다"는 방식을 그대로 화면으로 옮겼다 —
  **DB 에 직접 쓰지 않는다.** `/admin/products/new` 는 입력값으로 신상품 1건짜리 시드 SQL 을
  **브라우저에서 즉시** 만들어 보여줄 뿐이고, 사람이 내용을 읽고 `psql` 로 직접 적용한다.
  DB 쓰기 경로를 만들지 않은 이유: 이 세션엔 `DATABASE_URL` 도 로컬 DB 도 없어 실제 쓰기
  경로를 이번 실행에서 검증할 방법이 없었고, 원문 자체가 그 절충안(시드 SQL 생성)을 이미
  현재 방식으로 제시하고 있었다.
  SQL 생성 규칙(조건 등급 정규식 `DEFECT_RE`/`gradeOf`, 브랜드 slug 규칙 `brandSlug`, INSERT
  컬럼 목록)은 `scripts/gen_seed_sql.mjs` 와 의도적으로 판정 기준만 맞추고 코드는 공유하지
  않았다 — 이미 운영 DB 시드(009)를 만들어낸 검증된 스크립트를 이번 작업 때문에 건드리는
  위험을 지고 싶지 않았다. 새 모듈 `src/lib/product-seed-sql.ts` 는 pg 를 import 하지 않는
  순수 함수라 "use client" 컴포넌트(`AdminProductSqlForm.tsx`)에서 그대로 가져다 써도 빌드가
  안 깨진다.
  접근 제어: `users.role`(002 마이그레이션에 이미 `'customer'|'admin'` CHECK 로 존재하지만
  지금까지 이 값을 확인하는 화면이 하나도 없었다 — 006 마이그레이션 주석이 예고한
  "어드민(B단계)"이 이 화면이 처음이다)이 `'admin'` 인 로그인 사용자만 통과한다. 로그인
  안 했으면 `/login`, 로그인했지만 admin 이 아니면 홈으로 — `/account` 화면 접근 패턴을
  그대로 따랐다. `/account` 에 admin 에게만 보이는 진입 링크를 추가했다.
  **`/` `/m` 짝을 만들지 않았다** — 고객이 보는 화면이 아니라 운영자 한 명이 쓰는 내부
  도구라 이중 라우트 규칙(고객 화면 대상)의 적용 범위 밖이라고 판단했다. Tailwind 반응형
  클래스만으로 휴대폰에서도 입력 가능하게 했지만 실제 휴대폰 확인은 못 했다. 이 판단이
  틀렸다고 보면(=내부 도구도 이중 라우트 규칙 대상이라고 보면) `/m/admin/products/new` 를
  별도로 만들어야 한다.
  DB 쓰기 경로가 없는 것 자체가 한계다 — 실제로는 여전히 "SQL 을 만들어 사람이 psql 로
  적용" 이라 진짜 어드민 CRUD(등록 후 즉시 목록에 뜨는 것)는 아니다. 그 다음 단계(진짜 쓰기
  경로)는 이 세션이 DB 에 접속할 수 없어 검증이 불가능해 이번 범위에 넣지 않았다.
  브랜드 표기 충돌 방지는 못 넣었다 — 입력한 브랜드명이 기존 37건과 다른 표기(예: "Levis"
  vs "Levi's")면 브랜드가 두 행으로 갈린다. 폼에 경고 문구만 넣었고 기존 원장과 대조하는
  로직은 만들지 않았다(원장이 화면의 정본이 아니게 된 뒤로도 여전히 유일한 참조 데이터라
  이 화면이 서버에서 읽어와도 되는지 애매해서 범위 밖으로 뒀다).
  `Product.channel`(archive/shop) 은 DB 에 아직 컬럼이 없어(006/008/009 어디에도 없음) 폼에
  넣지 않았다 — 이 화면으로 등록한 상품은 전부 암묵적으로 archive 로 보인다.
  ...
  `/admin/products/new` 가 `ƒ (Dynamic)` 으로 생성됨을 빌드 로그에서 확인했다.
  `npm run start` 로컬 프로덕션 서버에서 확인한 것: `DATABASE_URL` 없는 이 세션에서
  `getSessionUser()` 가 null 을 돌려주므로 `/admin/products/new` 는 200 + "로그인이
  필요합니다" 안내만 뜨고 폼은 렌더되지 않는 것(로그인 벽이 실제로 작동), `/account` 는
  307(로그인 페이지로) — 로그인 상태에서의 실제 폼 동작·admin 아닌 로그인 사용자의 차단은
  DB 가 없어 이 세션에서 확인하지 못했다. `product-seed-sql.ts` 의 SQL 생성 로직 자체는
  Next 밖에서 별도 스크립트로 단위 검증했다 — 정상 입력 → `ON CONFLICT DO NOTHING` 5블록
  전부 포함한 올바른 SQL, 잘못된 입력(대문자 slug·음수 가격·이미지 0개) → 검증 에러 3건
  정확히 잡힘, "하자·스크래치" 문구 포함 조건문 → `used_fair` 등급 자동 판정까지 확인했다.
  실제 브라우저(휴대폰 포함)로는 보지 않았다 — curl·정적 HTML·standalone 로직 검증까지만 했다.

### 2026-08-27

- [x] `auto` **PR #32 후속 — `products.ts` 데이터 문자열 3종 정리 (사람이 PR #32 본문에 남긴 지시)**
  PR #32(홈 자리표시자 카피 제거 + 상세페이지 "판매자" 표현 정리)가 브라우저 편집이라 손대지
  못하고 "dev-loop 큐가 풀리면 이어서 처리한다"고 남긴 항목이다. 정식 `## 할 일` 항목은 아니지만
  방금 병합 대기 중이던 PR의 명시적 후속 지시라 우선 처리했다 — 원문이 첫 번째로 지목한
  `## 할 일` 최상단 "비밀번호 재설정"은 발송 채널·자격증명 결정이 선행돼야 해 이번 실행에서
  구조적으로 착수 불가능했다(아래 "비밀번호 재설정" 항목은 그대로 큐에 남겨둔다).
  `src/data/products.ts` 한 파일만 고쳤다(공유 데이터라 `/`·`/m` 양쪽에 한 번에 반영):
  1. `condition` 값의 `중고` → `세컨드핸드` 전량(37건 중 `중고`로 시작하던 36건 전부, 이미
     "빈티지"로 시작하던 1건은 그대로 둠).
  2. `measurements` 값의 `판매자 미기재 — ` 접두사 6건 제거(뒷문장은 그대로 유지).
  3. `note` 값의 "판매자 할인가 적용 중(이라)... 원가 변동/인상 가능" 4건 제거 — 그중 3건은
     그 문장이 note 전체라 `note` 필드째로 삭제했고, 1건(`carhartt-active-jacket-j130-m`)은
     같은 note 안에 있던 "가죽 패치에 사용감 있음." 하자 고지는 남기고 할인가 문장만 잘라냈다.
     PR #32 가 "우리에게 별도 원가가 있다는 내부정보 노출이라 우선순위 높음"이라 명시한 항목이다.
  PR #32 원문이 지목한 3종 패턴만 정확히 고쳤다 — 그 외에 파일 곳곳에 남아 있는 "판매자가
  ~기재/명시" 류 표현(hook·fit·fabric·recommendedFor 필드, 33건)은 **의도적으로 손대지 않았다**.
  PR #32 자체가 "판매자 고지 내용 자체는 남긴다 — 하자 고지를 지우면 청약철회 시 반환비용을
  우리가 문다"고 명시한 것과 같은 이유로, 이 문구들은 원매입처가 실측·소재를 확인 안 해줬다는
  사실을 손님에게 정직하게 알리는 provenance 성격이라 스코프 밖에서 임의로 지우지 않았다.
  `db/migrations/`·`ops/`·`.github/workflows/`·`AGENTS.md`·`next.config.ts`·결제/주문 코드·
  `package.json` 의존성 어느 것도 건드리지 않았다.
  `npm run lint && npm run build` 통과(기존 `DesktopViewLink.tsx` 무관 warning 1개만 남음),
  전체 라우트 표에서 상품 여섯 라우트 전부 `ƒ (Dynamic)` 유지 확인.
  `npm run start` 로컬 프로덕션 서버에서 확인한 것: `/archive`·`/m/archive` 둘 다 200, HTML 전체에서
  `중고`·`할인가` 문자열 **0건**(grep), `세컨드핸드` 72회(36건 × desktop/mobile 각 페이지) 등장.
  note 가 통째로 사라진 상품(`schott-618-perfecto-90s-m`) 상세페이지 200 + `할인가` 0건, note
  일부만 잘린 상품(`carhartt-active-jacket-j130-m`) 상세페이지에 "가죽 패치에 사용감 있음" 문구는
  남고 `할인가` 는 0건, measurements 접두사가 사라진 상품(`yiyae-washed-denim-trucker`) 상세페이지에
  `판매자 미기재` 0건을 확인했다.
