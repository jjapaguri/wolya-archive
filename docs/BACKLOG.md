...
- [ ] `review` **`mobile-layout` 브랜치를 main 에 병합한다 — 이게 다른 모든 항목의 선행 조건**
  모바일은 이미 완성돼 있다. 만들 것이 없고 **병합만 하면 된다.**
  `/m` 라우트, 모바일 컴포넌트 11개, 기기 감지 `src/proxy.ts` 가 브랜치에 들어 있다.
  지금 main 에는 이 중 아무것도 없어서 휴대폰으로 들어와도 데스크톱 레이아웃이 뜬다.
  인스타 유입은 사실상 전부 모바일이라 이게 최우선이다.
  - 병합 전까지 **다른 P0 항목을 먼저 하면 안 된다.** 새 라우트를 만들어도 모바일 짝을 만들 수 없기 때문이다
  - 병합 시 워크플로가 `Build`(브랜치) + `CI`(main) 둘이 되므로 하나로 정리한다
  - 병합 후 실제 휴대폰으로 `/` 접속 → `/m` 이동, 하단 "데스크톱 버전으로 보기" 왕복까지 확인
  - `review` 인 이유: 브랜치 병합과 워크플로 정리가 함께 필요해 되돌리기가 번거롭다
