...
- [ ] `review` **원본 매물 생존 체크 — 팔린 매물을 사이트에서 자동으로 내린다**
  ...
  > 2026-08-23 dev-loop (PR `auto/source-availability-check`): 읽기 전용 체크 스크립트
  > `scripts/check-source-availability.mjs` 를 만들어 이 항목의 절반(생존·가격 확인)만 구현했다.
  > `ops/` 는 건드리지 않았다 — cron 배선과 "자동으로 비공개 처리" 는 이번 범위 밖으로 남겨뒀다.
  > 이유: (1) `ops/automation.json` 의 `writes` 가 아직 `false` — 자동화가 사이트 데이터
  > (`src/data/products.ts`)에 쓰기를 하면 안 되는 상태다. (2) 실제로 29건을 fetch 해 확인해보니
  > fruitsfamily.com 페이지에 박힌 상품 상태 JSON 조각의 필드 순서가 상품마다 달라(`status` 뒤에
  > 오는 경우/`is_visible` 뒤에 오는 경우 두 가지를 봤다) 정규식 휴리스틱이 100% 신뢰할 수 없다 —
  > 이런 걸로 상품을 자동으로 내리면 오탐으로 멀쩡한 상품이 사라질 수 있다. 그래서 스크립트는
  > 판정이 애매하면 억지로 결론 내지 않고 "사람 확인 필요" 로만 남긴다.
  > 실행 결과(같은 날 실제 fetch): 29건 중 2건이 매입 당시 가격과 달랐다 — `carhartt-active-hood-tartan-j024`
  > 127,000→120,000원, `patagonia-micro-d-hoodie-xl` 120,000→100,000원(둘 다 더 싸졌음, 마진 손실 아님).
  > STATUS.md 가 적어둔 "현재 할인가 4건"(6c4i0·5ul0z·4sgch·6dpt5)은 지금도 할인 상태 그대로임을
  > 확인했다(할인가 = 기록된 `sourcePrice` 와 일치). 나머지는 원본 그대로 판매중.
  > 남은 일(사람이 설계): ops/ cron 에 하루 3회 배선, 결과를 어디에 쌓을지(ops 큐? DB?),
  > `writes` 켜진 뒤 자동 비공개 로직 자체.
