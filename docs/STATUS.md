...
사이트(`/`+`/m`)와 24시간 자동화는 돌고 있다. 앱↔DB 연결층(A1)은 병합됐고 운영 DB 에
마이그레이션 006~009 까지 적용됐다.
**지금 사람 손이 필요한 것은 둘이다.** (1) 로그인 PR 이 추가한 마이그레이션
`011_auth_sessions` 를 운영 DB 에 적용하는 것, (2) 소셜 로그인 앱 키를 서버
`.env.local` 에 넣는 것. 둘 다 안 해도 사이트는 지금 그대로 돌아간다 —
로그인 화면은 뜨고(소셜 버튼만 안 보인다), 011 적용 전에는 회원 기능만 안내 문구로 막힌다.
...
- 앱↔DB: A0(읽기 계층)에 이어 **A1 완료** — `/` `/m` `/shop` `/m/shop` `/product/[slug]`
  ...
  **006~009 는 운영 DB 에 적용 완료**(2026-08-26 사용자 확인). 폴백은 아직 코드에 남아 있고,
  걷어내는 것은 A2 항목이다.
...
| 소셜 로그인 앱 키 | 대기. `.env.local` 에 넣기 전까지 해당 버튼이 안 보인다. 등록 항목은 로그인 PR 본문 |
