...
입금 안내 → 주문조회까지 `/` 와 `/m` 양쪽에 있다(PR 대기).
**남은 것은 사람이 하는 절차 셋이다: (1) 운영 DB 에 마이그레이션 010 적용, (2) 입금 계좌
환경변수(`WOLYA_BANK_*`) 설정, (3) 입금 확인 → 주문 상태 진행 절차 정하기.**
셋 중 하나라도 없으면 주문은 받아지되 돈이 들어오는 길이 막힌다 — 010 이 없으면 주문 생성
자체가 실패하고, 계좌가 없으면 완료 화면이 "카카오톡으로 안내" 로 대체된다.
